...
- `main.yml`：存放 geerlingguy 角色所需的非敏感配置、共享 Docker 网络 (`docker_custom_*`) 以及监控栈的公共参数（如 `grafana_domain_name`）。Cloudflared 相关变量应放在各主机的 `host_vars/<hostname>/main.yml`，以便按主机精准控制。
